...
//// nRF receive from HUB ////
  ...
  while(network.available() == 0 ){
    ...
    Serial.println("Waiting to receive something on receiving end.");
    delay(500);
  }
...
// message indexes: CO2 = 0, Lux = 1, Temp = 2, RH = 3, Dust = 4
  ...
  if(temp_use == true) {
    ...
    t = sht31.readTemperature(); //get the temperature
  }else{
    t = 0;
  }
  ...
  if(RH_use == true) {
    ...
    h = sht31.readHumidity(); //get the humidity
  }else{
    h = 0;
  }
  ...
  if(dust_use == true) {
    ...
    dust = sht31.readHumidity(); //get the humidity
  }else{
    dust = 0;
  }
  }
  ...
  float t = sht31.readTemperature(); //get the temperature
  float h = sht31.readHumidity(); //get the relative humidity
