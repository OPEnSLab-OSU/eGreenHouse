...
String time_interval_str = "";// string version for the time between runs 
...
int time_interval = 0;// time to wait between runs down the HyperRail
...
void draw()
{
  ...
  //This will update the text on the screen automaticall for the intervals(meters)
  text("Current Interval: " + distance_interval + " m", 175, 465);
  
  text("Current Time Between Runs: " + time_interval + " s", 175,575);
 ...
 *           - It will also get values from the "Sensros Acrtived" section.
 * Return: void
 ****************************************************************/
                   ...
                   String stops, String time_interval)
{
  ...
  if (arduino_ready && data_ready)
  {
    String msg = "[O:" + option + ",S:" + steps + ",D:" + delay +
                 ",I:" + intervals_flag  + ",IS:" + interval_steps + ",ST:" + stops + ",TI:" + time_interval +
                 ",CO2:"  + co2_activated_str + ",Lux:" + lux_activated_str + ",Part:" +
                 particle_activated_str + ",H:" +  humidity_activated_str+ ",T:" + temperature_activated_str + "]";
  ...
  if(current_step == 0 && arduino_ready){
    ...
    if(data_ready == true){
      transmit_Data(option_str, running_steps_str, delay_time_str, intervals_str,
                    str(interval_steps),str(stops),str(time_interval));
  ...
  public void interval(String theInterval){
    ...
    if(length_input == true){
      ...
      if(float(theInterval)*1000 <= path_Length){
        ...
        println("The interval is greater than the path length.");
      }
    }else{
...
/*************************Function: time_interval()*****************
  * Description: This function will get the value from input box and then
                  update the corresonding string variable and float value
  * Parameters: string theTime_Interval
  * Return: VOID
  ******************************************************************/
public void Time_Interval(String theTime_Interval){
    ...
    //println( path_Length);//for testing
        ...
        println("The desired interval changed to: " + theTime_Interval);
        time_interval_str = theTime_Interval;
        time_interval = int(theTime_Interval);
        println("This is the interval:  " + time_interval);

      }



/************************Function: stepper_length()*****************************
