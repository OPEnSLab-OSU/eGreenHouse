...
// define all nodes
//const uint16_t LORA_HUB_ADDRESS = 00;  // hypeRail hub
//const uint16_t LORA_NODE_ADDRESS = 01; // eGreenhouse sensor node
//const uint16_t LORA_NODE_2_ADDRESS = 02; // ethernet hub
...
void transmit_LoRa_sensors()
{
  //Serial.println("begin LoRa transmission");
  ...
  //    is_sent = lora_send_bundle(&bndl, LORA_NODE_ADDRESS);
  //    delay(1000);
  //  }
  ...
  if (is_sent) {
    ...
    //Serial.println("bundle sent to sensors!");
  }
  ...
  while (  (!manager.available()) && millis() - start_listening < 20000  ) {
  ...
  lora_receive_bundle(&sensor_bundle);
  if ( (sensor_bundle.size() > 0) && (sensor_bundle.size() < 1000) ) {
    Serial.println(bndl.size());
    Serial.println(sensor_bundle.size());
    print_bundle(&sensor_bundle);
    Serial.println("Received sensor data!");
  }
  else {
    Serial.println("Failed to receive sensor data");
  }

  bool is_uploaded = lora_send_bundle(&sensor_bundle, LORA_NODE_2_ADDRESS);
  if (is_uploaded) {
    Serial.println("Send to ethernet hub success!");
  }
  else {
    //lora_send_bundle(&sensor_bundle, LORA_NODE_2_ADDRESS);
    Serial.println("Failed to reach ethernet hub");
  }
...
void intervals_travelHyperRail(long steps_total, int delay_time, long interval_steps, int stops )
{
  for (;;) {
    ...
    //Serial.println("begin interval loop");
    transmit_LoRa_sensors();
    //int count = count+=1;
    //Serial.print("Index: ");
    ...
    for (int s = 1; s <= stops; s++) {
      ...
      //wait 1 minute before requesting data
      //delay(60000);
      transmit_LoRa_sensors();
    }
