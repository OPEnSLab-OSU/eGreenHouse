...
const uint16_t LORA_NODE_ADDRESS = 01; 
const uint16_t LORA_NODE_2_ADDRESS = 02; // Address of our node in Octal format
const uint16_t LORA_HUB_ADDRESS = 00; 
...
bool lora_send_bundle(OSCBundle *bndl, uint16_t destination)
{
  ...
  if (is_sent) {
    LOOM_DEBUG_Println("Sent bundle through LoRa!");
     ...
     LOOM_DEBUG_Println("Failed to send");
  }
